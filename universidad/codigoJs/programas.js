 ...
 var listar = function(accion){
 	var table = $('#datatableProgramas').DataTable({
 		destroy:true,
 		dom: 'Blfrtip',
 		buttons: [
 		...
 		"ajax":{
 			"method":"POST",
 			"url":"registros/registro_programas.php",
 			"data": {"accion":accion}
 		},
 		"columns":[
 		{"data":"pro_id"},
 ...
 var dataeditar = function(body,table){
 	$(body).on("click","button.Editarprograma",function(){
 		var data = table.row($(this).parents("tr")).data();
 		console.log(data);
 		$("#id").val(data.pro_id);
    $("#codigoprograma2").val(data.pro_codigo);
 		$("#nombreprograma2").val(data.pro_nombre);
